...
fn update_history(in_hist: &mut [u8], out_hist: &mut [u8], last_in: &[u8], last_out: &[u8])
{
	...
	if in_hist_len > 0
	{
		let start_pos = if last_in.len() > in_hist_len { last_in.len() - in_hist_len } else { last_in.len() };
		cryptoutil::copy_memory(&last_in[start_pos..], in_hist);
	}
	let out_hist_len = out_hist.len();
	if out_hist_len > 0
	{
		let start_pos = if last_out.len() > out_hist_len { last_out.len() - out_hist_len } else { last_out.len() };
		cryptoutil::copy_memory(&last_out[start_pos..], out_hist);
	}
}
...
impl <P: BlockProcessor, X: PaddingProcessor> BlockEngine<P, X>
{
	...
	/// and output history.
	fn new_with_history(processor: P, padding: X, block_size: usize, in_hist: Vec<u8>, out_hist: Vec<u8>) -> BlockEngine<P, X>
	{
		BlockEngine
		{
			in_hist: in_hist,
	...
	fn fast_mode<R: ReadBuffer, W: WriteBuffer>(&mut self, input: &mut R, output: &mut W) -> BlockEngineState
	{
		fn has_next<R: ReadBuffer, W: WriteBuffer>(input: &mut R, output: &mut W, block_size: usize) -> bool
		{
	...
	fn process<R: ReadBuffer, W: WriteBuffer>(&mut self, input: &mut R, output: &mut W, eof: bool) -> Result<BufferResult, SymmetricCipherError>
	{
		...
		fn process_scratch<P: BlockProcessor, X: PaddingProcessor>(me: &mut BlockEngine<P, X>)
		{
...

			{
				...
				me.processor.process_block(&me.in_hist[..], &me.out_hist[..], next_in, next_out);
		...
		loop
		{
			match self.state
			{
				...
				BlockEngineState::NeedInput =>
				{
					...
					{
						if eof { self.state = BlockEngineState::LastInput }
						else { return Ok(BufferUnderflow) }
					}
				}
...
impl <T: BlockEncryptor, X: PaddingProcessor> EcbEncryptor<T, X>
{
	...
	pub fn new(algo: T, padding: X) -> EcbEncryptor<T, EncPadding<X>>
	{
		...
		EcbEncryptor { block_engine: BlockEngine::new(processor, EncPadding::wrap(padding), block_size) }
	}
...
impl <T: BlockDecryptor, X: PaddingProcessor> EcbDecryptor<T, X>
{
	...
	pub fn new(algo: T, padding: X) -> EcbDecryptor<T, DecPadding<X>>
	{
		...
		EcbDecryptor { block_engine: BlockEngine::new(processor, DecPadding::wrap(padding), block_size) }
	}
...
impl <T: BlockEncryptor, X: PaddingProcessor> CbcEncryptor<T, X>
{
	...
	{
		let block_size = algo.block_size();
		let processor = CbcEncryptorProcessor { algo: algo, temp: repeat(0).take(block_size).collect() };
		CbcEncryptor
		{
			block_engine: BlockEngine::new_with_history(processor,
...
impl <T: BlockDecryptor, X: PaddingProcessor> CbcDecryptor<T, X>
{
	...
	{
		let block_size = algo.block_size();
		let processor = CbcDecryptorProcessor { algo: algo, temp: repeat(0).take(block_size).collect() };
		CbcDecryptor
		{
			block_engine: BlockEngine::new_with_history(processor,
