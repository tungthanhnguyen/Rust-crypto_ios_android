...
impl <'a> ReadBuffer for RefReadBuffer<'a>
{
	...
	fn remaining(&self) -> usize
	{
		...
		if buff_len > self.pos { buff_len - self.pos }
		else { 0 }
	}
	...
	fn rewind(&mut self, distance: usize)
	{
		if self.pos > distance { self.pos -= distance }
		else { self.pos = 0 }
	}

	fn truncate(&mut self, amount: usize)
	{
		let mut buff_len = self.buff.len();
		if buff_len > amount
		{
			buff_len -= amount;
			self.buff = &self.buff[..buff_len]
		}
	}
	...
	{
		let buff_len = self.buff.len();
		let last_idx = if count < buff_len { count } else { buff_len };
		if self.pos < last_idx { &self.buff[self.pos..last_idx] }
		else { &self.buff[last_idx..last_idx] }
	}

	fn take_next(&mut self, count: usize) -> &[u8]
	{
		let buff_len = self.buff.len();
		let last_idx = if self.pos + count < buff_len { self.pos + count } else { buff_len };
		let r = if self.pos < last_idx { &self.buff[self.pos..last_idx] } else { &self.buff[last_idx..last_idx] };
		if self.pos + count < buff_len { self.pos += count }
		else { self.pos = buff_len }
		r
	}
}
...
impl OwnedReadBuffer
{
	...
	{
		let len = buff.len();
		OwnedReadBuffer { buff: buff, len: len, pos: 0 }
	}
...
impl ReadBuffer for OwnedReadBuffer
{
	...
	fn remaining(&self) -> usize
	{
		if self.len > self.pos { self.len - self.pos }
		else { 0 }
	}
	...
	fn truncate(&mut self, amount: usize)
	{
		if self.len > amount { self.len -= amount }
		else { self.len = 0 }
	}
...
impl <'a> RefWriteBuffer<'a>
{
	...
	{
		let len = buff.len();
		RefWriteBuffer { buff: buff, len: len, pos: 0 }
	}
}
...
impl <'a> WriteBuffer for RefWriteBuffer<'a>
{
	...
	{
		let buff_len = self.buff.len();
		let last_idx = if self.pos < buff_len { self.pos } else { buff_len };
		RefReadBuffer::new(&self.buff[..last_idx])
	}
	...
	fn take_next(&mut self, count: usize) -> &mut [u8]
	{
		let buff_len = self.buff.len();
		let last_idx = if self.pos + count < buff_len { self.pos + count } else { buff_len };
		let begin_idx = if self.pos < last_idx { self.pos } else { last_idx };
		if self.pos + count < buff_len { self.pos += count }
		else { self.pos = buff_len }
		&mut self.buff[begin_idx..last_idx]
	}
...
impl <'a> BorrowedWriteBuffer<'a>
{
	fn new(parent: &mut OwnedReadBuffer) -> BorrowedWriteBuffer
	{
		...
		BorrowedWriteBuffer { parent: parent, pos: 0, len: buff_len }
	}
}
...
impl <'a> WriteBuffer for BorrowedWriteBuffer<'a>
{
	...
	fn rewind(&mut self, distance: usize)
	{
		...
		if self.parent.len > distance { self.parent.len -= distance }
		else { self.parent.len = 0 }
	}
	...
	fn peek_read_buffer(&mut self) -> RefReadBuffer
	{
		let buff_len = self.parent.buff.len();
		let last_idx = if self.pos < buff_len { self.pos } else { buff_len };
		RefReadBuffer::new(&self.parent.buff[..last_idx])
	}
	...
	fn take_next<>(&mut self, count: usize) -> &mut [u8]
	{
		...
		{
			self.pos = buff_len;
			self.parent.len = buff_len
		}

		&mut self.parent.buff[begin_idx..last_idx]
	}
...
impl OwnedWriteBuffer
{
	...
	{
		let len = buff.len();
		OwnedWriteBuffer { buff: buff, len: len, pos: 0 }
	}

	pub fn into_read_buffer(self) -> OwnedReadBuffer
	{
		let buff_len = self.buff.len();
		let pos = if self.pos < buff_len { self.pos } else { buff_len };
