...
use crate::aes::KeySize::{KeySize128, KeySize192, KeySize256, KeySize512, KeySize1024};
...
impl AesNiEncryptor
{
	pub fn new(key_size: KeySize, key: &[u8]) -> AesNiEncryptor
	{
		...
		let (rounds, setup_function): RoundSetupInfo = match key_size
		{
			...
			KeySize512  => (22, setup_working_key_aesni_512),
			KeySize1024 => (38, setup_working_key_aesni_1024)
		};
...
extern
{
	...
	fn rust_crypto_aesni_setup_working_key_512(key: *const u8, round_key: *mut u8);

	fn rust_crypto_aesni_setup_working_key_1024(key: *const u8, round_key: *mut u8);

	fn rust_crypto_aesni_encrypt_block(rounds: u8,
	                                   input: *const u8,
	                                   round_keys: *const u8,
																		 output: *mut u8);

	fn rust_crypto_aesni_decrypt_block(rounds: u8,
	                                   input: *const u8,
	                                   round_keys: *const u8,
	                                   output: *mut u8);
}
...
fn setup_working_key_aesni_512(key: &[u8], key_type: KeyType, round_key: &mut [u8])
{
	unsafe
	{
		rust_crypto_aesni_setup_working_key_512(key.as_ptr(), round_key.as_mut_ptr());
...
fn setup_working_key_aesni_1024(key: &[u8], key_type: KeyType, round_key: &mut [u8])
{
	unsafe
	{
		rust_crypto_aesni_setup_working_key_1024(key.as_ptr(), round_key.as_mut_ptr());
...
fn encrypt_block_aesni(rounds: u8, input: &[u8], round_keys: &[u8], output: &mut [u8])
{
	unsafe
	{
		rust_crypto_aesni_encrypt_block(rounds, input.as_ptr(), round_keys.as_ptr(), output.as_mut_ptr());
	}
}
...
fn decrypt_block_aesni(rounds: u8, input: &[u8], round_keys: &[u8], output: &mut [u8])
{
	unsafe
	{
		rust_crypto_aesni_decrypt_block(rounds as u8, input.as_ptr(), round_keys.get_unchecked(round_keys.len() - 16), output.as_mut_ptr());
	}
}
